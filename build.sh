...
build_builder_image() {
  echo
  echo "=== Build diametertools_builder image ==="
  echo
  _read image_tag
  _read base_os
  _read base_tag
  ...
  docker build --rm ${DBUILD_XTRA_OPTS} ${bargs} -f Dockerfile.build -t ${registry}/diametertools_builder:"${image_tag}" . || return 1
  set +x
}
...
build_project() {
  ...
  echo
  echo "=== Build diametertools project ==="
  echo
  _read base_tag
  _read make_procs
  _read build_type
  ...
  rm -f CMakeCache.txt
  # shellcheck disable=SC2086
  docker run --rm -it -u "$(id -u):$(id -g)" ${envs} -v "${PWD}":/code -w /code ${registry}/diametertools_builder:"${base_tag}" || return 1
  # shellcheck disable=SC2086
  docker run --rm -it -u "$(id -u):$(id -g)" ${envs} -v "${PWD}":/code -w /code ${registry}/diametertools_builder:"${base_tag}" "" doc || return 1
  set +x
}

build_project_image() {
  echo
  echo "=== Build diametertools image ==="
  echo
  _read image_tag
  _read base_tag
  _read make_procs
  ...
  rm -f CMakeCache.txt
  # shellcheck disable=SC2086
  docker build --rm ${DBUILD_XTRA_OPTS} ${bargs} -t ${registry}/diametertools:"${image_tag}" . || return 1
  set +x
}
